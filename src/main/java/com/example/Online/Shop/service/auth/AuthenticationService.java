...
public class AuthenticationService {
    ...
    // Autentica al usuario utilizando su correo y contraseña y generar tokens de acceso y refresco
    ...
    // Registra un nuevo usuario en el sistema, verifica si ya existe, genera un token de verificacion y envia un correo para verificar la cuenta
    ...
    private RegisterResponse sendVerificationEmail(Customer user){
        ...
        return new RegisterResponse(null, null, "User registered successfully. Please check your email to verify your account.");
    }
    ...
    public ResponseEntity<String> verifyUser(String token){
        try {
            ...
            log.info("End - AuthenticationService -> verifyUser() - {} has been successfully verified", customer.getEmail());
    ...
    public CustomerResponse getAuthenticatedUser(){
        String username = getUsernameFromPrincipal(SecurityContextHolder.getContext().getAuthentication().getPrincipal());
        log.info("AuthenticatedUser: {}", username);
        Customer findedCustomer = repository.findByUsername(username).orElseThrow(() -> new RuntimeException("User not found"));
