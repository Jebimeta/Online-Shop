...
public class CustomerVerificationTokenServiceImpl implements CustomerVerificationTokenService {
	...
	public Customer verifyCustomerByToken(String token) {
		...
		if (verifyCustomer.isPresent()) {
			...
			customer.setStatus(true); // Actualizamos el estado del campo a true para
										// indicar que el usuario ha sido verificado
			customer.setVerificationToken(null); // Vaciamos este campo, pues ya no nos
													// sirve una vez el usuario ya ha sido
													// verificado
